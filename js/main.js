...
fetch(BREEDS_URL)
    ...
    select.addEventListener('change', event => {
        ...
        dogeInfo.assignMF()
        dogeInfo.assignAge()
        dogeInfo.assignLikes()
        dogeInfo.assignDislikes()
        dogeInfo.assignFunFact()
        
    })

    const img = document.querySelector('.dog-img')
    
    const getDogeImg = url => {
    ...
    const dogeInfo = {
        ...
        assignName(array) {
            this.rname = array[Math.floor(Math.random()*array.length)] //going to give a number bacj that is array length -1
        ...
        yatesShuffle(array) {
            ...
            assignFunFact() {
                ...
                document.getElementById("fun-fact").innerHTML = `Additonal Info: ${this.fact}`
            }
        }